...
import { apiDetailTransaction, apiUpdateTransaction } from '../../../utils/api';
...
export default class DetailSalesOrder extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
    ...
    this.renderAcceptOrder = this.renderAcceptOrder.bind(this);
  }
  ...
  render() {
    const { loading, loadingAccept } = this.state;
    return (
      <View style={{ flex: 1 }}>
        <AwareScroll>
          ...
          <SeparatorHorizontal />
          {loading ? <View /> : this.renderAcceptOrder()}
        </AwareScroll>
  ...
  renderAcceptOrder() {
    const { data, loadingAccept } = this.state;
    if (
      (data && data.status && data.status.toLowerCase() === 'unpaid') ||
      (data && data.status && Number(data.status) === 0)
    )
      return (
  ...
  async acceptOrder() {
    ...
    try {
      ...
      const response = await apiUpdateTransaction(data.id_transaction, '', '1');
  ...
  renderDetailOrder() {
    const { data } = this.state;
    if (data && data.order_date && data.status) {
      return (
        <Card style={{ margin: 8 }}>
  ...
  renderPaymentMethod() {
    const { data } = this.state;
    if (data && data.payment && data.payment.bank_code) {
      return (
        <View style={{ padding: 8, width: '100%' }}>
          <Card>
            <MediumText style={{ flex: 1, fontWeight: 'bold' }}>
